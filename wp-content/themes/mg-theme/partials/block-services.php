<section class="services">
  <div class="container">
    <h2 class="title title-white" data-anim="fadeInRight"><span>Our</span> Services</h2>
    <div class="services__slider">
      <div data-anim="fadeInUp">
        <div class="services__item">
          <div class="services__info">
            <img src="<?php echo IMG; ?>/service-img1.png" alt="">
            <h6>Donec justo sapien</h6>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam eu faucibus magna. Fusce et odio nec tellus egestas volutpat.</p>
            <button class="btn btn-yellow" data-modal="services_1">read more</button>
          </div>
        </div>
      </div>
      <div data-anim="fadeInUp" data-anim-delay=".2">
        <div class="services__item">
          <div class="services__info">
            <img src="<?php echo IMG; ?>/service-img2.png" alt="">
            <h6>Donec justo sapien</h6>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam eu faucibus magna. Fusce et odio nec tellus egestas volutpat.</p>
            <button class="btn btn-yellow" data-modal="services_2">read more</button>
          </div>
        </div>
      </div>
      <div data-anim="fadeInUp" data-anim-delay=".4">
        <div class="services__item">
          <div class="services__info">
            <img src="<?php echo IMG; ?>/service-img3.png" alt="">
            <h6>Donec justo sapien</h6>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam eu faucibus magna. Fusce et odio nec tellus egestas volutpat.</p>
            <button class="btn btn-yellow" data-modal="services_3">read more</button>
          </div>
        </div>
      </div>
      <div>
        <div class="services__item">
          <div class="services__info">
            <img src="<?php echo IMG; ?>/service-img2.png" alt="">
            <h6>Donec justo sapien</h6>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam eu faucibus magna. Fusce et odio nec tellus egestas volutpat.</p>
            <button class="btn btn-yellow" data-modal="services_4">read more</button>
          </div>
        </div>
      </div>
    </div>
    <div class="services__callback" data-anim="fadeInUp">
      <h4>Call me back</h4>
      <form class="form" action="/">
        <input id="tel" type="tel" placeholder="phone" maxlength="14" maxlength="14" required />
        <input type="submit" value="send">
      </form>
    </div>
  </div>
</section>

<section class="modal" id="services_1">
  <div class="modal__content">
    <button class="modal__close">
      <svg width="84" height="85" viewBox="0 0 84 85" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M81.2187 15.2188C84.4739 11.9635 84.4739 6.67708 81.2187 3.42188C77.9635 0.166667 72.677 0.166667 69.4218 3.42188L41.9999 30.8698L14.552 3.44792C11.2968 0.192708 6.01034 0.192708 2.75513 3.44792C-0.500081 6.70312 -0.500081 11.9896 2.75513 15.2448L30.203 42.6667L2.78117 70.1146C-0.474039 73.3698 -0.474039 78.6562 2.78117 81.9115C6.03638 85.1667 11.3228 85.1667 14.578 81.9115L41.9999 54.4635L69.4478 81.8854C72.703 85.1406 77.9895 85.1406 81.2447 81.8854C84.4999 78.6302 84.4999 73.3438 81.2447 70.0885L53.7968 42.6667L81.2187 15.2188Z" fill="#FDC236"/></svg>
    </button>
    <div class="modal__info">
      <img class="modal__img" src="<?php echo IMG; ?>/service-img1.png" alt="">
      <div class="modal__text">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Cumque, eius velit ipsam eum laboriosam quibusdam quam. Non distinctio sint beatae.</div>
    </div>
  </div>
</section>
<section class="modal" id="services_2">
  <div class="modal__content">
    <button class="modal__close">
      <svg width="84" height="85" viewBox="0 0 84 85" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M81.2187 15.2188C84.4739 11.9635 84.4739 6.67708 81.2187 3.42188C77.9635 0.166667 72.677 0.166667 69.4218 3.42188L41.9999 30.8698L14.552 3.44792C11.2968 0.192708 6.01034 0.192708 2.75513 3.44792C-0.500081 6.70312 -0.500081 11.9896 2.75513 15.2448L30.203 42.6667L2.78117 70.1146C-0.474039 73.3698 -0.474039 78.6562 2.78117 81.9115C6.03638 85.1667 11.3228 85.1667 14.578 81.9115L41.9999 54.4635L69.4478 81.8854C72.703 85.1406 77.9895 85.1406 81.2447 81.8854C84.4999 78.6302 84.4999 73.3438 81.2447 70.0885L53.7968 42.6667L81.2187 15.2188Z" fill="#FDC236"/></svg>
    </button>
    <div class="modal__info">
      <img class="modal__img" src="<?php echo IMG; ?>/service-img1.png" alt="">
      <div class="modal__text">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Cumque, eius velit ipsam eum laboriosam quibusdam quam. Non distinctio sint beatae.</div>
    </div>
  </div>
</section>
<section class="modal" id="services_3">
  <div class="modal__content">
    <button class="modal__close">
      <svg width="84" height="85" viewBox="0 0 84 85" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M81.2187 15.2188C84.4739 11.9635 84.4739 6.67708 81.2187 3.42188C77.9635 0.166667 72.677 0.166667 69.4218 3.42188L41.9999 30.8698L14.552 3.44792C11.2968 0.192708 6.01034 0.192708 2.75513 3.44792C-0.500081 6.70312 -0.500081 11.9896 2.75513 15.2448L30.203 42.6667L2.78117 70.1146C-0.474039 73.3698 -0.474039 78.6562 2.78117 81.9115C6.03638 85.1667 11.3228 85.1667 14.578 81.9115L41.9999 54.4635L69.4478 81.8854C72.703 85.1406 77.9895 85.1406 81.2447 81.8854C84.4999 78.6302 84.4999 73.3438 81.2447 70.0885L53.7968 42.6667L81.2187 15.2188Z" fill="#FDC236"/></svg>
    </button>
    <div class="modal__info">
      <img class="modal__img" src="<?php echo IMG; ?>/service-img1.png" alt="">
      <div class="modal__text">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Cumque, eius velit ipsam eum laboriosam quibusdam quam. Non distinctio sint beatae.</div>
    </div>
  </div>
</section>
<section class="modal" id="services_4">
  <div class="modal__content">
    <button class="modal__close">
      <svg width="84" height="85" viewBox="0 0 84 85" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M81.2187 15.2188C84.4739 11.9635 84.4739 6.67708 81.2187 3.42188C77.9635 0.166667 72.677 0.166667 69.4218 3.42188L41.9999 30.8698L14.552 3.44792C11.2968 0.192708 6.01034 0.192708 2.75513 3.44792C-0.500081 6.70312 -0.500081 11.9896 2.75513 15.2448L30.203 42.6667L2.78117 70.1146C-0.474039 73.3698 -0.474039 78.6562 2.78117 81.9115C6.03638 85.1667 11.3228 85.1667 14.578 81.9115L41.9999 54.4635L69.4478 81.8854C72.703 85.1406 77.9895 85.1406 81.2447 81.8854C84.4999 78.6302 84.4999 73.3438 81.2447 70.0885L53.7968 42.6667L81.2187 15.2188Z" fill="#FDC236"/></svg>
    </button>
    <div class="modal__info">
      <img class="modal__img" src="<?php echo IMG; ?>/service-img1.png" alt="">
      <div class="modal__text">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Cumque, eius velit ipsam eum laboriosam quibusdam quam. Non distinctio sint beatae.</div>
    </div>
  </div>
</section>

